...
export const useRealtimeChat = (projectId: string | null, currentUser: User | null) => {
  ...
  const fetchMessages = useCallback(
    async (projId: string) => {
      if (fetchingRef.current || !supabase || !isSupabaseConfigured) return;
    ...
    const channel = supabase
      ...
      .subscribe((status) => {
        ...
        if (status === 'CHANNEL_ERROR' || status === 'TIMED_OUT') {
          // Kick a refresh to avoid stale UI
  ...
  // Lightweight polling fallback for environments where websockets are blocked
  useEffect(() => {
    if (!projectId || !supabase || !isSupabaseConfigured) return;
    const interval = setInterval(() => {
      // If not actively subscribed, or as a periodic refresh, fetch
      if (realtimeStatus !== 'SUBSCRIBED') {
        fetchMessages(projectId);
      }
    }, 8000);
    return () => clearInterval(interval);
  }, [projectId, realtimeStatus, fetchMessages]);
